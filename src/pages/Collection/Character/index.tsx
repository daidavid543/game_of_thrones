...
export default function CharacterList(props: { collection: string }) {
    const [rows, setRows] = useState<Array<Character>>([]);
    ...
    useEffect(() => {
        listCollection(collection, {}).then(value => {
            if (value && value.length) {
                setRows(value);
                setGenders(prevState => {
                    // Filter unique
                    const items: Array<string> = [];
                    value.map((item: any) => !items.includes(item.gender) ? items.push(item.gender) : null);
                    return items;
                });
            }
        });
    ...
    const onDetail = (item: Character) => {
        const id = item.url.toString().split('/').pop();
        history.push(`/collection/${collection}/detail/${id}`)
    }

    const filterItems = (): Array<Character> => {
        return rows.filter((value: Character) => {
            // Filter search
            ...
            // Filter gender
            return !(!!gender && !value.gender.toLocaleLowerCase().includes(gender.toLocaleLowerCase()));
        });
    }
                ...
                <div className="card-toolbar" data-select2-id="select2-data-83-brtm">
                    ...
                    <div className="my-1">
                        <select
                            ...
                            <option key={'-1'} value={''}>All Gender</option>
                            {
                                genders.map((value: string, index: number) => {
                                    return (
            ...
            <div className="card-body pt-3 pb-0 mt-n3">
                <div className="table-responsive">
                    <table className="table table-borderless align-middle">
                        ...
                        <tbody>
                        {
                            filterItems().map((value: Character, index: number) => {
                                return (
                                    <tr key={index.toString()}>
                                        ...
                                        <td className="px-0">
                                            ...
                                            <span
                                                className="text-muted fw-bold d-block mt-1">{value.culture}</span>
                                        </td>
                                        <td/>
                                        <td className="text-end">
                                            <span className="text-gray-800 fw-bolder d-block fs-6">Gender</span>
                                            <span
                                                className="text-muted fw-bold d-block mt-1 fs-7">{value.gender}</span>
                                        </td>
