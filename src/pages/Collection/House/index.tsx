...
export default function HouseList(props: { collection: string }) {
    const [rows, setRows] = useState<Array<House>>([]);
        ...
        listCollection(collection, {}).then(value => {
            if (value && value.length) {
                ...
                setRegions(prevState => {
                    ...
                    value.map((item: any) => !items.includes(item.region) ? items.push(item.region) : null);
                    return items;
                });
            }
        });
    ...
    const onDetail = (item: House) => {
        const id = item.url.toString().split('/').pop();
        history.push(`/collection/${collection}/detail/${id}`)
    }

    const filterItems = (): Array<House> => {
        return rows.filter((value: House) => {
            // Filter search
            ...
            // Filter region
            return !(!!region && !value.region.toLocaleLowerCase().includes(region.toLocaleLowerCase()));
        });
    }
                ...
                <div className="card-toolbar" data-select2-id="select2-data-83-brtm">
                    ...
                    <div className="my-1">
                        <select
                            ...
                            <option key={'-1'} value={''}>All Region</option>
                            {
            ...
            <div className="card-body pt-3 pb-0 mt-n3">
                <div className="table-responsive">
                    <table className="table table-borderless align-middle">
                        ...
                        <tbody>
                        {
                            filterItems().map((value: House, index: number) => {
                                return (
                                    <tr key={index.toString()}>
                                        ...
                                        <td className="px-0">
                                            ...
                                            <span
                                                className="text-muted fw-bold d-block mt-1">{value.coatOfArms}</span>
                                        </td>
                                        <td/>
                                        <td className="text-end">
                                            <span className="text-gray-800 fw-bolder d-block fs-6">Region</span>
                                            <span
                                                className="text-muted fw-bold d-block mt-1 fs-7">{value.region}</span>
                                        </td>
